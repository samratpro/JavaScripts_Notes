...
async function saveSessionAndExit(): Promise<void> {
  try {
    if (context) {
      ...
      console.log(`📁 File: ${storageStateFile}`);
...
async function runBrowser(): Promise<void> {
  try {
    ...
    console.log('\n🌐 Browser is running.');
    console.log('📌 Do your login and work. Close browser when done.\n');
...
async function controlChrome(): Promise<void> {
  ...
  console.log('✅ Session saved to storage_state.json.');
...
async function reuseSession(): Promise<void> {
  if (isStorageStateValid(storageStateFile)) {
    ...
    console.log('✅ Navigated to Pinterest using saved session. Add logic here...');
...
```
## Another Uses of Fix Tab
```ts
async function fetchUrl(page: any, url: string): Promise<void> {
  try {
...
const main = async (): Promise<void> => {
  ...
  let startPage = await loadProgress(TOTAL_PAGES);
  const browser = await chromium.launch({headless: false});
  const context = await browser.newContext();
  const pages = [await context.newPage(), await context.newPage(),];

  for (let i = startPage; i <= TOTAL_PAGES; i += 2) {
    const currentBatch = [];
    // Page 1 of batch
    const url1 = `https://www.pagesjaunes.fr/annuaire/chercherlespros?quoiqui=notaire&page=${i}`;
    currentBatch.push(fetchUrl(pages[0], url1));
    // Page 2 of batch (if exists)
    if (i + 1 <= TOTAL_PAGES) {
      const url2 = `https://www.pagesjaunes.fr/annuaire/chercherlespros?quoiqui=notaire&page=${i + 1}`;
      currentBatch.push(fetchUrl(pages[1], url2));
    }
    await Promise.all(currentBatch);
    const lastProcessed = i + (i + 1 <= TOTAL_PAGES ? 1 : 0);
    await saveProgress(lastProcessed, TOTAL_PAGES);
    console.log(`Progress: ${lastProcessed}/${TOTAL_PAGES} pages scraped\n`);
    await new Promise(resolve => setTimeout(resolve, 2000 + Math.random() * 3000));
  }
  await browser.close();
  console.log(`\nScraping completed! All links saved to: ${OUTPUT_FILE}`);
};
