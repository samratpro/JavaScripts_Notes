...
async function scrapeNotary(url: string): Promise<boolean> {
  try {
    // Remove query parameters from URL (everything after ?)
    const cleanUrl = url.split('?')[0] || url;
    const response = await scraper.get(cleanUrl);
    const $ = cheerio.load(response.body);
    ...
    // Check if we actually got valid data (name should not be N/A)
    if (name === 'N/A') {
      console.log(`⚠️  No valid data found for: ${cleanUrl}`);
      return false; // Failed - no data extracted
    }
...
async function main() {
  ...
  let skip: number = 0; // Track consecutive skips
  ...
  for (let i = startIndex - 1; i < allLinks.length; i += CONCURRENCY) {
    ...
    // Scrape all URLs in batch concurrently and track results
    const results = await Promise.all(batch.map(url => scrapeNotary(url)));

    // Count how many failed in this batch
    const failedCount = results.filter(success => !success).length;
    const successCount = results.filter(success => success).length;
    
    console.log(`📊 Batch result: ${successCount} success, ${failedCount} failed`);
    
    if (failedCount > 0) {
      skip += failedCount;
      console.log(`⚠️  Skip count: ${skip} (added ${failedCount} from this batch)`);
      
      // If we've skipped 6 or more, sleep for 10 seconds
      if (skip >= 3) {
        console.log(`\n⏸️  Reached ${skip} skips. Sleeping for 10 seconds to avoid rate limiting...`);
        await Sleep(20000);
        skip = 0; // Reset skip counter
        console.log('✅ Sleep complete. Resuming scraping...\n');
      }
    } else {
      // If batch was successful, we can optionally reduce skip count
      if (skip > 0) {
        skip = Math.max(0, skip - 1);
        console.log(`✅ All success! Skip count reduced to: ${skip}`);
      }
    }
