...
## 01. util-text.ts – Text File Utils
```ts
// util-text.ts
...
export async function saveProgress(PROGRESS_FILE: string, page: number): Promise<void> {
  const progress: Progress = {
    currentPage: page,
    totalPages: undefined, // optional
...
export async function loadProgress(totalPages: number, PROGRESS_FILE: string): Promise<number> {
  try {
    const data = await fs.readFile(PROGRESS_FILE, 'utf-8');
    const json = JSON.parse(data) as Progress;
    ...
    // File missing or invalid → start from 1
  }
  return 1;
}
...
## 02. util-csv.ts – Fast CSV Utils (2025 Standard)
```ts
// util-csv.ts
import fs from 'fs/promises';
import path from 'path';
...
// ─────────────────────────────────────
// Shared: Ensure dir + Progress (same as in util-text.ts)
export async function ensureDir(filePath: string): Promise<void> {
  await fs.mkdir(path.dirname(filePath), { recursive: true });
}

export interface Progress { currentPage: number; lastRun?: string }
export async function saveProgress(PROGRESS_FILE: string, page: number): Promise<void> {
  const progress: Progress = { currentPage: page, lastRun: new Date().toISOString() };
  await ensureDir(PROGRESS_FILE);
  await fs.writeFile(PROGRESS_FILE, JSON.stringify(progress, null, 2), 'utf-8');
}
...
// ─────────────────────────────────────
// CSV: Read (streaming + typed)
export async function readCsvFile<T extends Record<string, any>>(
  filePath: string
): Promise<T[]> {
  return new Promise((resolve, reject) => {
    if (!require('fs').existsSync(filePath)) return resolve([]);
...
```
## 03. util-json.ts – JSON Utils (Perfect for Scrapers)
```ts
// util-json.ts
import fs from 'fs/promises';
import path from 'path';
